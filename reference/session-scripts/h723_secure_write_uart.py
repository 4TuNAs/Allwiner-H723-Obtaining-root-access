...
Write strategy:
  1) Read + hash the live 128 KiB region.
  2) Refuse unless it exactly matches the known original dump.
  3) Save a local backup before any write.
  4) Write only the six changed 4 KiB blocks.
     New item blocks first, backup map next, primary map LAST.
  5) Read back every 4 KiB block immediately.
  6) Read back the whole 128 KiB region and require exact SHA-256 match.
  7) Never reboots the device automatically.
"""
...
# Known-good live dump supplied from this exact H723 board.
EXPECTED_ORIGINAL_SHA256 = (
    "cc86b0255f07e9cec320285df83899a51354698c70003a8b1b1e56c807820490"
)

# Candidate generated and validated from that dump.
EXPECTED_CANDIDATE_SHA256 = (
    "28a017814d6af136ab18f5f76396af4cd37f20cd407abfb613b0e4968c27fcc4"
)

# Safe commit order:
#   item3 primary + backup
#   item4 primary + backup
#   backup map
#   primary map LAST
...
class UARTShell:
        ...
        while time.time() < deadline:
            ...
            if line == end:
                ...
                if len(data) != expected:
                    raise RuntimeError(
                        f"Read length mismatch at LBA {lba}: "
                        f"{len(data)} != {expected}"
                    )
                return data
            ...
            if collecting and line:
                if not all(c.isalnum() or c in "+/=" for c in line):
                    raise RuntimeError(
                        f"Unexpected non-base64 UART line while reading LBA {lba}: "
                        f"{line!r}"
                    )
    ...
    def preflight_write_open(self):
        ...
        # count=0: opens the block device for writing but transfers zero bytes.
        cmd = (
            f"dd if=/dev/null of=/dev/block/mmcblk0 "
            f"bs=512 seek={SECURE_LBA} count=0 conv=notrunc "
        ...
        if rc_text != "0":
            raise RuntimeError(
                f"Write-open preflight failed (rc={rc_text!r}). "
                "No eMMC data was written."
            )
    ...
    def preflight_base64_decode(self):
        ...
        cmd = (
            f"echo QQ== | base64 -d | "
            f"dd of=/dev/null bs=1 count=1 2>/dev/null; "
            f"rc=$?; echo {marker}$rc"
        )
        ...
        if rc_text != "0":
            raise RuntimeError(
                f"base64 -d preflight failed (rc={rc_text!r}). "
                "No eMMC data was written."
            )
    ...
    def write_sector(self, lba: int, sector_data: bytes):
        ...
        # One sector per command keeps the shell command line short
        # and gives us a clear success/failure marker.
        cmd = (
            f"echo {encoded} | base64 -d | "
            f"dd of=/dev/block/mmcblk0 bs=512 seek={lba} "
            f"count=1 conv=notrunc 2>/dev/null; "
            f"rc=$?; echo {marker}$rc"
        )
        ...
        if rc_text != "0":
            raise RuntimeError(
                f"Sector write failed at LBA {lba} (rc={rc_text!r})"
            )
...
def local_candidate_checks(candidate: bytes):
    if len(candidate) != SECURE_SIZE:
        raise SystemExit(
            f"Candidate size is {len(candidate)} bytes; expected {SECURE_SIZE}"
        )
    ...
    if h != EXPECTED_CANDIDATE_SHA256:
        raise SystemExit(
            "REFUSING: candidate SHA-256 does not match the validated H723 image."
        )


def audit_live(shell: UARTShell, backup_dir: Path):
    print(
        f"Reading live Secure Storage: "
        f"LBA {SECURE_LBA}..{SECURE_LBA + SECURE_SECTORS - 1}"
    )
    ...
    if h != EXPECTED_ORIGINAL_SHA256:
        raise SystemExit(
            "\nREFUSING TO WRITE: live Secure Storage differs from the known "
            "original dump. Nothing has been written."
        )
...
def write_block(shell: UARTShell, candidate: bytes, block_index: int):
    ...
    print(
        f"\nWriting 4 KiB block {block_index} "
        f"(offset 0x{off:05x}, LBA {start_lba}-{start_lba + 7})"
    )
    ...
    if readback != block:
        raise RuntimeError(
            f"READ-BACK MISMATCH for block {block_index}. "
            "STOPPING immediately. Do NOT reboot the board."
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Safeguarded H723 Secure Storage writer over recovery UART"
    )
    ap.add_argument("port", help="UART port, e.g. COM5")
    ap.add_argument(
        "candidate",
        help="Validated 128 KiB H723 Secure Storage candidate image",
    )
    ap.add_argument("--baud", type=int, default=115200)
    ap.add_argument(
        "--backup-dir",
        default=".",
        help="Directory for automatic pre-write backup (default: current dir)",
    )
    ap.add_argument(
        "--write",
        action="store_true",
        help="ACTUALLY WRITE eMMC. Without this flag the script is read-only.",
    )
    args = ap.parse_args()
    ...
    try:
        ...
        for b in BLOCK_ORDER:
            ...
            print(
                f"  block {b:2d}: offset 0x{off:05x}, "
                f"LBA {lba}-{lba + 7}"
            )
        ...
        print(
            "\nCommit order deliberately writes new item copies first, "
            "backup map next, primary map LAST."
        )
        ...
        if final != candidate:
            ...
            raise RuntimeError(
                f"FINAL VERIFY FAILED. Read-back saved to {bad}. "
                "DO NOT REBOOT THE BOARD."
            )
